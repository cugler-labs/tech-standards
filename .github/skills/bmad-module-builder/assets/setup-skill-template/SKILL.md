...
After both merge scripts complete successfully, remove the installer's package directories. Skills and agents in these directories are already installed at `.github/skills/` — the `_bmad/` directory should only contain config files.
...
```bash
uv run ./scripts/cleanup-legacy.py --bmad-dir "{project-root}/_bmad" --module-code {module-code} --also-remove _config --skills-dir "{project-root}/.github/skills"
```

The script verifies that every skill in the legacy directories exists at `.github/skills/` before removing anything. Directories without skills (like `_config/`) are removed directly. If the script exits non-zero, surface the error and stop. Missing directories (already cleaned by a prior run) are not errors — the script is idempotent.
