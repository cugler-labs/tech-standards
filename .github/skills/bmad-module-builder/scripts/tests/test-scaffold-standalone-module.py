...
def test_basic_scaffold():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (tmp / "distribution" / "marketplace.json").is_file()
...
def test_marketplace_json_content():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        marketplace = json.loads(
            (tmp / "distribution" / "marketplace.json").read_text()
        )
...
def test_custom_marketplace_dir():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Should be at custom location, not default parent
            assert (custom_dir / "distribution" / "marketplace.json").is_file()
            assert not (tmp / "distribution" / "marketplace.json").exists()
            assert data["marketplace_json"] == str((custom_dir / "distribution" / "marketplace.json").resolve())
