...
Skills may run in environments where Python or `uv` is unavailable (for example, a web-based runtime). Scripts should be the fast, reliable path — but the skill must still deliver its outcome when execution is not possible.
