...
An adapter is a JSON file the scripts read. Runtime-specific adapters are supplied by the environment:

```json
{
  "name": "runtime",
  "invocation": ["runtime", "{prompt}"],
...
| `skill_dir` | no | directory under the cwd where the runtime discovers skills. Default `.github/skills`. Used to stage the skill under test and trigger mode's synthetic skill. |
...
Nothing found means the run degrades to staging-only (cases prepared, results recorded as skipped).
...
The subprocess environment is built from scratch, never inherited, so host shell config, memories, and tokens cannot bias the result. It contains `PATH`, a fresh empty `HOME` at `<case>/.home`, the adapter's auth variable when set non-empty on the host, and any `env_passthrough` keys present on the host. There is no container, no terminal emulation, and no credential file staging.
