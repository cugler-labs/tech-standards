...
skill_dir comes from the adapter (default ".github/skills"). Without this
...
holds PATH, a fresh empty HOME at <case>/.home, the adapter's auth_env var
ONLY if set non-empty in the host env
...
references/platform-adapter.md:
  ...
  invocation      : argv template. "{prompt}" -> composed case prompt,
    ...
    auth_env        : optional env var name carrying auth.
...
def build_case_env(adapter: Mapping | None, home_dir: Path,
    ...
    the adapter's auth var ONLY when set
    ...
    if auth_env:
        value = host_env.get(str(auth_env))
        if value:
            env[str(auth_env)] = value
...
def run_case(case: dict, case_dir: Path, run_dir: Path,
    ...
    if skill_path is not None:
        skills_subdir = (adapter or {}).get("skill_dir", ".github/skills")
