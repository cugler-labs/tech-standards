...
trigger-specific keys to the core ones:
  ...
  skill_dir  : path under the staging cwd where a skill is discovered, e.g.
               ".github/skills". The runner writes the synthetic skill there.
...
auth var only when set, adapter env_passthrough
...
def run_query_once(query: str, skill_name: str, description: str,
    ...
    skill_subdir = adapter.get("skill_dir", ".github/skills")
...
def main(argv: list[str] | None = None) -> int:
    ...
    def run_one(idx: int, q: dict, run_idx: int) -> tuple[int, bool]:
        ...
        try:
            ...
            shutil.rmtree(stage / adapter.get("skill_dir", ".github/skills").split("/")[0],
                          ignore_errors=True)
